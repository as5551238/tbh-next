...
const TYPE_CFG: Record<string, { label: () => string; color: string; icon: typeof Target }> = {
  goal: { label: () => t('bookmarks.typeGoal'), color: 'bg-purple-500/20 text-purple-400', icon: Target },
  task: { label: () => t('bookmarks.typeTask'), color: 'bg-green-500/20 text-green-400', icon: ListChecks },
  doc: { label: () => t('bookmarks.typeDoc'), color: 'bg-blue-500/20 text-blue-400', icon: FileText },
  knowledge: { label: () => t('bookmarks.typeKnowledge'), color: 'bg-orange-500/20 text-orange-400', icon: BookOpen },
  other: { label: () => t('bookmarks.typeOther'), color: 'bg-surface-2 text-text-3', icon: Bookmark },
};
...
export default function BookmarksContent() {
  ...
  const handleAdd = async () => {
    ...
    success(t('bookmarks.bookmarkAdded'));
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-sm font-bold">{t('bookmarks.title')}</span>
        <span className="text-[10px] text-text-3">{t('bookmarks.countLabel', { count: filtered.length })}</span>
        ...
        {ALL_TYPES.map((ft) => {
          ...
          return (
            <button key={ft} onClick={() => setFilterType(ft)} className={cn('shrink-0 rounded-full px-3 py-1 text-[11px] font-medium transition-all', filterType === ft ? 'bg-primary/10 text-primary-2' : 'text-text-3 hover:bg-surface-2')}>
              {ft === 'all' ? t('bookmarks.filterAll') : cfg?.label() ?? ft}
            </button>
          );
        })}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-text-3">
          ...
          <span className="text-xs">{filterType === 'all' ? t('bookmarks.emptyAll') : t('bookmarks.emptyFiltered')}</span>
        </div>
      ) : (
      ...
      <Modal open={addModal.open} onClose={addModal.closeModal} title={t('bookmarks.addModalTitle')}
        footer={
        ...
        <ModalField label={t('bookmarks.titleLabel')}>
          <input className={inputCls} placeholder={t('bookmarks.titlePlaceholder')} value={newItem.title} onChange={(e) => setNewItem((p) => ({ ...p, title: e.target.value }))} />
        </ModalField>
        ...
        <ModalField label={t('bookmarks.typeLabel')}>
          <select className={inputCls} value={newItem.target_type} onChange={(e) => setNewItem((p) => ({ ...p, target_type: e.target.value }))}>
            {Object.entries(TYPE_CFG).map(([k, v]) => <option key={k} value={k}>{v.label()}</option>)}
          </select>
        </ModalField>
        <ModalField label={t('bookmarks.categoryLabel')}>
          <input className={inputCls} placeholder={t('bookmarks.categoryPlaceholder')} value={newItem.category} onChange={(e) => setNewItem((p) => ({ ...p, category: e.target.value }))} />
        </ModalField>
      </Modal>
    </div>
  );
