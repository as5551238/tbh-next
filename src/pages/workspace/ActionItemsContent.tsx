/**
 * ActionItemsContent — Action item management (core business loop page)
 *
 * Features:
 * 1. Action item list — filter by source/status/priority
 * 2. Create action item — sources: manual/review/deviation/AI suggested
 * 3. Convert to task — loop: ActionItem → Task (auto-mark closed_loop)
 * 4. Deviation alert display — associated deviation alerts
 * 5. Goal linkage — action item's parent goal at a glance
 */
...
// --- Status/Source/Priority label config ---

const STATUS_CFG: Record<string, { label: () => string; color: string; icon: typeof Circle }> = {
  open: { label: () => t('actionItems.statusOpen'), color: 'text-text-3', icon: Circle },
  in_progress: { label: () => t('actionItems.statusInProgress'), color: 'text-blue-400', icon: Clock },
  completed: { label: () => t('actionItems.statusCompleted'), color: 'text-success', icon: CheckCircle2 },
  cancelled: { label: () => t('actionItems.statusCancelled'), color: 'text-text-3', icon: Circle },
};

const SOURCE_CFG: Record<string, { label: () => string; color: string }> = {
  manual: { label: () => t('actionItems.sourceManual'), color: 'bg-gray-500/20 text-gray-400' },
  review: { label: () => t('actionItems.sourceReview'), color: 'bg-purple-500/20 text-purple-400' },
  deviation: { label: () => t('actionItems.sourceDeviation'), color: 'bg-red-500/20 text-red-400' },
  ai_suggested: { label: () => t('actionItems.sourceAiSuggested'), color: 'bg-blue-500/20 text-blue-400' },
};

const PRIORITY_CFG: Record<string, { label: () => string; color: string }> = {
  critical: { label: () => t('actionItems.priorityCritical'), color: 'text-red-400' },
  high: { label: () => t('actionItems.priorityHigh'), color: 'text-orange-400' },
  medium: { label: () => t('actionItems.priorityMedium'), color: 'text-yellow-400' },
  low: { label: () => t('actionItems.priorityLow'), color: 'text-text-3' },
};
...
export default function ActionItemsContent() {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Sort: priority > status > created_at
  ...
  const goalTitle = (goalId: string | null) => {
    ...
    return goals.find((g) => g.id === goalId)?.title ?? t('actionItems.unknownGoal');
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
        {[
          { label: t('actionItems.statusOpen'), value: stats.open, color: 'text-text-2', icon: Circle },
          { label: t('actionItems.statusInProgress'), value: stats.inProgress, color: 'text-blue-400', icon: Clock },
          { label: t('actionItems.statusCompleted'), value: stats.completed, color: 'text-success', icon: CheckCircle2 },
          { label: t('actionItems.statClosedLoopRate'), value: actionItems.length > 0 ? `${Math.round(stats.closedLoop / actionItems.length * 100)}%` : '-', color: 'text-primary-2', icon: Zap },
          { label: t('actionItems.statUnresolvedAlerts'), value: stats.unresolvedAlerts, color: stats.unresolvedAlerts > 0 ? 'text-danger' : 'text-text-3', icon: AlertTriangle },
        ].map((s) => (
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-sm font-bold">{t('actionItems.title')}</span>
        <span className="text-[10px] text-text-3">{t('actionItems.itemCount', { count: filteredItems.length })}</span>
      ...
      {showFilters && (
        <div className="flex flex-wrap items-center gap-3 rounded-xl bg-surface-2 px-3 py-2">
          <span className="text-[11px] text-text-3">{t('actionItems.statusLabel')}</span>
          <select className={cn(inputCls, 'text-[11px] py-1')} value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
            <option value="all">{t('actionItems.filterAll')}</option>
            <option value="open">{t('actionItems.statusOpen')}</option>
            <option value="in_progress">{t('actionItems.statusInProgress')}</option>
            <option value="completed">{t('actionItems.statusCompleted')}</option>
            <option value="cancelled">{t('actionItems.statusCancelled')}</option>
          </select>
          <span className="text-[11px] text-text-3">{t('actionItems.sourceLabel')}</span>
          <select className={cn(inputCls, 'text-[11px] py-1')} value={filterSource} onChange={(e) => setFilterSource(e.target.value)}>
            <option value="all">{t('actionItems.filterAll')}</option>
            <option value="manual">{t('actionItems.sourceManual')}</option>
            <option value="review">{t('actionItems.sourceReview')}</option>
            <option value="deviation">{t('actionItems.sourceDeviation')}</option>
            <option value="ai_suggested">{t('actionItems.sourceAiSuggested')}</option>
          </select>
        </div>
      )}

      {/* Deviation alerts (unresolved) */}
      {stats.unresolvedAlerts > 0 && (
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <AlertTriangle size={14} className="text-danger" />
            <span className="text-xs font-bold text-danger">{t('actionItems.deviationAlerts')}</span>
          ...
          {alerts.filter((a) => !a.is_resolved).slice(0, 3).map((alert) => (
            <div key={alert.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-danger/30 bg-danger/5 px-3 py-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                {alert.goal_id && <div className="text-[10px] text-text-3">{t('actionItems.goalLabel', { title: goalTitle(alert.goal_id) ?? '' })}</div>}
              </div>
              {!alert.action_item_id && (
                <button
                  ...
                  onClick={async () => {
                    await addActionItem({
                      title: `${t('actionItems.handleDeviationPrefix')}${alert.message.slice(0, 30)}`,
                      source: 'deviation',
                      source_id: alert.id,
                ...
                >
                  {t('actionItems.generateActionItem')}
                </button>
              )}
            </div>
          ))}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-text-3">
          ...
          <span className="text-xs">{t('actionItems.emptyState')}</span>
          <span className="text-[10px]">{t('actionItems.emptyHint')}</span>
        </div>
      ) : (
        <div className="space-y-2">
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                  <button
                    ...
                    title={t('actionItems.detailComment')}
                  >
                  ...
                  {item.status !== 'completed' && !item.closed_loop && (
                    <button
                      ...
                      title={t('actionItems.convertToTaskTitle')}
                    >
                      <ArrowRight size={10} />
                  ...
                  {item.closed_loop && (
                    <button className="flex flex-wrap items-center gap-0.5 rounded-lg bg-success/10 px-2 py-1 text-[10px] font-semibold text-success" title={t('actionItems.closedLoopDone')}>
                       <ExternalLink size={10} />
                       {t('actionItems.closedLoopDone')}
                    </button>
                  )}
                  <button
      ...
      <Modal open={addModal.open} onClose={addModal.closeModal} title={t('actionItems.newActionItemTitle')}
        footer={
      ...
      >
        <ModalField label={t('actionItems.titleLabel')}>
          <input className={inputCls} placeholder={t('actionItems.titlePlaceholder')} value={newItem.title} onChange={(e) => setNewItem((p) => ({ ...p, title: e.target.value }))} />
        </ModalField>
        <ModalField label={t('actionItems.descLabel')}>
          <textarea className={cn(inputCls, 'min-h-[60px]')} placeholder={t('actionItems.descPlaceholder')} value={newItem.description} onChange={(e) => setNewItem((p) => ({ ...p, description: e.target.value }))} />
        </ModalField>
        <ModalField label={t('actionItems.linkedGoalLabel')}>
          <select className={inputCls} value={newItem.goal_id || '__EMPTY__'} onChange={(e) => setNewItem((p) => ({ ...p, goal_id: e.target.value === '__EMPTY__' ? '' : e.target.value }))}>
            <option value="__EMPTY__">{t('actionItems.noLinkedGoal')}</option>
            {goals.map((g) => <option key={g.id} value={g.id}>{g.title}</option>)}
          </select>
        </ModalField>
        <ModalField label={t('actionItems.priorityLabel')}>
          <select className={inputCls} value={newItem.priority} onChange={(e) => setNewItem((p) => ({ ...p, priority: e.target.value }))}>
            <option value="low">{t('actionItems.priorityLow')}</option>
            <option value="medium">{t('actionItems.priorityMedium')}</option>
            <option value="high">{t('actionItems.priorityHigh')}</option>
            <option value="critical">{t('actionItems.priorityCritical')}</option>
          </select>
        </ModalField>
        <ModalField label={t('actionItems.dueDateLabel')}>
          <input type="date" className={inputCls} value={newItem.due_date} onChange={(e) => setNewItem((p) => ({ ...p, due_date: e.target.value }))} />
        </ModalField>
      </Modal>

      {/* Convert to task modal */}
      <Modal open={convertModal.open} onClose={convertModal.closeModal} title={t('actionItems.convertModalTitle')}
        footer={
          <div className="flex flex-wrap gap-2">
            <button className={btnSecondary} onClick={convertModal.closeModal}>{t('common.cancel')}</button>
            <button className={btnPrimary} onClick={handleConvert}>{t('actionItems.confirmConvert')}</button>
          </div>
        }
      >
        <div className="mb-3 rounded-lg bg-surface-2 p-3">
          ...
          {convertItem?.goal_id && <div className="text-[10px] text-primary-2 mt-1">{t('actionItems.linkedGoalWith', { title: goalTitle(convertItem.goal_id) ?? '' })}</div>}
        </div>
        <p className="text-[11px] text-text-3 mb-3">
          {t('actionItems.convertDesc')}
        </p>
        <ModalField label={t('actionItems.taskDueDateLabel')}>
          <input type="date" className={inputCls} value={convertDueDate} onChange={(e) => setConvertDueDate(e.target.value)} />
        </ModalField>
      </Modal>

      {/* Action item details/comments modal */}
      <Modal open={detailModal.open} onClose={detailModal.closeModal} title={t('actionItems.detailTitle')}
        footer={
          <div className="flex flex-wrap gap-2">
            <button className="mr-auto rounded-lg px-3 py-1.5 text-[11px] font-semibold text-danger hover:bg-danger/10" onClick={() => { if (detailItem) { removeActionItem(detailItem.id); detailModal.closeModal(); setDetailItem(null); } }}>{t('common.delete')}</button>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              ...
              {detailItem.closed_loop && (
                <span className="rounded-full bg-success/10 px-1.5 py-0.5 text-[10px] font-semibold text-success">{t('actionItems.closedLoop')}</span>
              )}
            </div>
