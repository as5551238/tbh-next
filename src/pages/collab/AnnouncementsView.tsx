...
const ANN_FIELDS: FieldDef[] = [
  { key: 'title', label: () => t('announcements.fieldTitle'), type: 'text' },
  { key: 'content', label: () => t('announcements.fieldContent'), type: 'textarea' },
  { key: 'priority', label: () => t('announcements.fieldPriority'), type: 'select', options: [
    { value: 'top', label: () => t('announcements.priorityTop') },
    { value: 'normal', label: () => t('announcements.priorityNormal') },
    { value: 'info', label: () => t('announcements.priorityInfo') },
  ]},
  { key: 'department', label: () => t('announcements.fieldDepartment'), type: 'text' },
];
...
export default function AnnouncementsView() {
  ...
  async function handleCreate() {
    ...
    await addAnnouncement({
      ...
      author: t('announcements.me'),
      department: form.department,
      views: 0,
      comments: 0,
      time: t('announcements.justNow'),
    });
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <div className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-3">
        <span className="text-sm font-bold">{t('announcements.title')}</span>
        ...
        <button onClick={createModal.openModal} className="ml-auto rounded-lg bg-primary/10 px-3 py-1 text-[11px] font-semibold text-primary-2 hover:bg-primary/20 transition-colors">{t('announcements.newAnnouncement')}</button>
      </div>
          ...
          <div key={ann.id} className={cn('rounded-xl border border-border bg-surface p-4 transition-all hover:border-border-2 hover:shadow-lg cursor-pointer',
            ...
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              <span className={cn('rounded-full px-1.5 py-0.5 text-[8px] font-bold', PRIORITY_STYLES[ann.priority])}>
                {ann.priority === 'top' ? t('announcements.priorityTop') : ann.priority === 'normal' ? t('announcements.priorityNormal') : t('announcements.priorityInfo')}
              </span>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={createModal.closeModal}>
          <div className="w-96 rounded-xl border border-border bg-surface-2 p-3 md:p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-bold">{t('announcements.createTitle')}</span>
              <button onClick={createModal.closeModal} aria-label={t('announcements.close')} className="text-text-3 hover:text-text"><X size={16} /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-[10px] text-text-3 mb-1 block">{t('announcements.titleLabel')}</label>
                <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder={t('announcements.titlePlaceholder')} className={inputCls + ' w-full'} />
              </div>
              <div>
                <label className="text-[10px] text-text-3 mb-1 block">{t('announcements.contentLabel')}</label>
                <textarea value={form.content} onChange={(e) => setForm((f) => ({ ...f, content: e.target.value }))} placeholder={t('announcements.contentPlaceholder')} rows={4} className={inputCls + ' w-full resize-none'} />
              </div>
              <div>
                <label className="text-[10px] text-text-3 mb-1 block">{t('announcements.priorityLabel')}</label>
                <select value={form.priority} onChange={(e) => setForm((f) => ({ ...f, priority: e.target.value }))} className={inputCls + ' w-full'}>
                  <option value="info">{t('announcements.priorityInfo')}</option>
                  <option value="normal">{t('announcements.priorityNormal')}</option>
                  <option value="top">{t('announcements.priorityTop')}</option>
                </select>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2 mt-4">
              <button onClick={handleCreate} disabled={!form.title.trim() || !form.content.trim()} className={`${btnPrimary} disabled:opacity-40`}>{t('announcements.publish')}</button>
